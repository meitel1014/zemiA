...
rddKingaku = rdd.map(lambda x: (x,1)).reduceByKey(lambda x,y:x+y)
sortedRdd = rddKingaku.sortByKey().mapValues(lambda x: x-1)

kingakuPair = rddKingaku.collect()

print(kingakuPair)
print(kingakuPair[3][1])
#目標金額となる組み合わせを求める
S = 0 #現在の合計金額
...
#1週目
for i in range(MAX):
    while (S < X) and (kaitou[MAX-1-i] < kingakuPair[MAX-1-i][1]):
...
while S != X:
    ...
    S = S - (min+1)
    for i in range(min):
        while(S < X) and (kaitou[min-1-i] < kingakuPair[min-1-i][1]):
